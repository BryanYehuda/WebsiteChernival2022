...
test('replace with loop', () => {
  expect(1);
  before_and_after_replace({ loop: true });
});

test('replace without loop', () => {
  expect(1);
...
test('remove with loop', () => {
  expect(3);

  before_and_after_remove({ loop: true });
});

test('remove without loop', () => {
  expect(3);
...
function before_and_after_remove(options) {
  const simple = $('#simple');
  const one = simple.clone().removeAttr('id').insertAfter('#simple');
  const two = one.clone().insertAfter(one);
  const all = two.clone().insertAfter(two);
...
test('remove and add with loop', () => {
  expect(1);

  before_and_after_remove_add({ loop: true });
});

test('remove and add without loop', () => {
  expect(1);
...
function before_and_after_remove_add(options) {
  const simple = $('#simple');
  const simpleClone = $('#simple').clone().removeAttr('id').insertAfter('#simple');
...
test('invalidate', () => {
  ...
  const carousel = $('#simple').owlCarousel().data('owl.carousel');
...
test('destroy', () => {
  ...
  const simple = $('#simple');
  const expected = simple.get(0).outerHTML.replace(/\s{2,}/g, '');
